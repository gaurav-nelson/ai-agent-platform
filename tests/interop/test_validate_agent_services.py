...
def test_validate_sample_api(openshift_dyn_client):
    ...
    assert len(api_pods) >= 1, "No running sample-api pod found in ai-agent namespace"
...
def test_validate_agent_chat_ui(openshift_dyn_client):
    ...
    assert len(ui_pods) >= 1, "No running agent-chat-ui pod found in ai-agent namespace"
...
def test_validate_pgvector(openshift_dyn_client):
    ...
    assert len(pg_pods) >= 1, "No running pgvector pod found in ai-agent namespace"
...
def test_validate_bee_agent_health_endpoint(openshift_dyn_client):
    ...
    assert (
        "healthy" in result.stdout.lower()
...
def test_validate_bee_agent_lists_agents(openshift_dyn_client):
    ...
    assert (
        "cluster-health" in result.stdout
