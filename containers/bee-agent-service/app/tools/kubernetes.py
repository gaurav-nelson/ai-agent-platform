...
class GetPodsTool(Tool[GetPodsInput, ToolRunOptions, StringToolOutput]):
    name = "GetPods"
    description = (
        "List pods in a Kubernetes namespace with their name,"
...
class GetNodesTool(Tool[GetNodesInput, ToolRunOptions, StringToolOutput]):
    ...
    async def _run(
        ...
        for node in nodes.items:
            ...
            lines.append(
                f"{name}  Ready={ready}"
                f"  Roles={','.join(roles) or 'worker'}"
                f"  CPU={cpu}  Memory={mem}  GPU={gpu}"
            )
        return StringToolOutput("\n".join(lines))
...
class GetEventsInput(BaseModel):
    namespace: str = Field(
        default="default",
        description="Kubernetes namespace to list events from",
    )
    limit: int = Field(
        default=20,
...
class DescribeResourceInput(BaseModel):
    resource_type: str = Field(
        description="Type of resource: pod, node, deployment, service"
    )
    name: str = Field(description="Name of the resource to describe")
    namespace: str = Field(
        default="default",
        description="Namespace of the resource (not used for nodes)",
    )


class DescribeResourceTool(
    Tool[DescribeResourceInput, ToolRunOptions, StringToolOutput]
):
    name = "DescribeResource"
    description = (
        "Get detailed information about a specific Kubernetes"
        " resource (pod, node, deployment, or service)"
    )
    ...
    async def _run(
        ...
        try:
            if rt == "pod":
                ...
                obj = await asyncio.to_thread(
                    _apps_v1.read_namespaced_deployment,
                    input.name,
                    input.namespace,
                )
                ...
                obj = await asyncio.to_thread(
                    _core_v1.read_namespaced_service,
                    input.name,
                    input.namespace,
                )
                ...
                return StringToolOutput(
                    f"Unsupported resource type: {rt}."
    ...
    def _format_deployment(self, dep: Any) -> str:
        return (
            ...
            f"Replicas: {dep.status.ready_replicas or 0}"
            f"/{dep.spec.replicas}\n"
